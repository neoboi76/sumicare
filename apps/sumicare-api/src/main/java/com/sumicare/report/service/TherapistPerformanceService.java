...
public class TherapistPerformanceService {
    ...
    public record TherapistPerformance(
            UUID therapistId, String nickname, BigDecimal revenue, BigDecimal commissions,
            BigDecimal tips, long servicesRendered, long specificRequests,
            List<NameCount> topClients, List<NameCount> topServices) {}
    ...
    public TherapistPerformanceReport report(UUID organizationId, LocalDate from, LocalDate to) {
        ...
        for (Session session : sessionRepository.findAllByOrganizationIdAndStartedAtBetween(organizationId, start, end)) {
            ...
            String clientName = booking == null || booking.getClientNickname() == null ? "(Walk-in)" : booking.getClientNickname();
        ...
        for (Map.Entry<UUID, Accumulator> e : byTherapist.entrySet()) {
            ...
            String nickname = therapistRepository.findById(e.getKey()).map(t -> t.getNickname()).orElse("Therapist");
            rows.add(new TherapistPerformance(e.getKey(), nickname, acc.revenue, acc.commissions, acc.tips,
